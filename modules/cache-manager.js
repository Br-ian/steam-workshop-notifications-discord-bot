...
export const createModsCache = function() {
    return util.fileExists(modsCacheFile)
        .then(exists => { return !exists ? util.writeFile(modsCacheFile, '{}') : Promise.resolve(exists) })
}
...
export const createNotificationsCache = function() {
    return util.fileExists(notificationsCacheFile)
        .then(exists => { return !exists ? util.writeFile(notificationsCacheFile, '{}') : Promise.resolve(exists) })
}
...
export const createSpotRepCache = function() {
    return util.fileExists(spotRepCacheFile)
        .then(exists => { return !exists ? util.writeFile(spotRepCacheFile, `{ "lastUpdate": "${moment().format()}"}`) : Promise.resolve(exists) })
}
